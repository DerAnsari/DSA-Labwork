...
template <typename UF> class HexBoard {
private:
  const int n_;
  ...
  int id(int r, int c) const { return r * n_ + c; }

  // True if the two virtual boundary nodes of `player` are connected.
  bool connected(int a, int b) { return uf_.find(a) == uf_.find(b); }
...
public:
  HexBoard(int n)
      : n_(n), board_(n, std::vector<int>(n, -1)), uf_(n * n + 4), top_(n * n),
        bottom_(n * n + 1), left_(n * n + 2), right_(n * n + 3) {}
  ...
  bool place(int r, int c, int player) {
    ...
    //       are now connected.
    if (r < 0 || r >= n_ || c < 0 || c >= n_ || board_[r][c] != -1) {
      return false;
    }

    board_[r][c] = player;

    // we take the hexagons surrounding out hexagon and if we find any we can
    // unify them
    for (auto [x, y] : dirs()) { // why const or refrence???
      int newRow = r + x;
      int newCols = c + y;

      // if it is same then we merge
      if (newRow < n_ && newRow >= 0 && newCols < n_ && newCols >= 0 &&
          board_[newRow][newCols] == player) {
        uf_.unify(id(r, c), id(newRow, newCols));
      }
    }

    if (player == 0) {
      if (r == 0)
        uf_.unify(id(r, c), top_);

      if (r == n_ - 1)
        uf_.unify(id(r, c), bottom_);

    } else if (player == 1) {

      if (c == 0)
        uf_.unify(id(r, c), left_);

      if (c == n_ - 1)
        uf_.unify(id(r, c), right_);
    }
